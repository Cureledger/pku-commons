...
**New here? Start with [`docs/RELIABILITY.md`](docs/RELIABILITY.md).** It breaks phe-estimator
...
There are two kinds of contribution, and each gets reviewed differently
([`docs/PEER-REVIEW.md`](docs/PEER-REVIEW.md)):

- **Layer 2: you're improving an estimate.** Prove it with a benchmark number. CI gates merges.
  Your run must not regress the baseline.
- **Layer 1: you're adding or changing knowledge** (a food-list row, a phe-per-gram rule). Cite an
...
3. **Score it against the baseline** (this is exactly what CI runs):
   ...
   ```
   Exit code is non-zero if you regress (MAE up or within-band percent down). A green run means
   you're good.
...
2. **Regression gate**: the deterministic rubric gets re-scored against the baseline. A
