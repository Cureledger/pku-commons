...
Those map onto two long-established review traditions, **legal review** and **scientific review**, and we
use both, applied to different objects.
...
- **Resolution** is recorded: the decision, the reasoning, and the authority relied on become part of the
  permanent record. The issue to resolution trail is the audit trail.
...
- Contributions are merged only when they **improve the score without regressing** other cases, enforced
  in **CI**, not by reviewer goodwill.
...
- **Quality is guaranteed by the eval, not the author.** Every version is scored against the public
  benchmark, so an app can change maintainers, or lose its original developer entirely, and families still
